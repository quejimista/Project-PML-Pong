...
def train_model(env, model_name, thresh):
    ...
    # create
    if model_name == "dqn":
        model = DQN(config["policy_type"], env, verbose=0, tensorboard_log=f"runs/{run.id}", device=DEVICE, normalize_images=False)
    elif model_name == "a2c":
        model = A2C(config["policy_type"], env, verbose=0, tensorboard_log=f"runs/{run.id}", device=DEVICE, normalize_images=False)
    elif model_name == "ppo":
        model = PPO(config["policy_type"], env, verbose=0, tensorboard_log=f"runs/{run.id}", device=DEVICE, normalize_images=False)
    else:
...
def eval_model(env, model_name):
    ...
    # load model
    if model_name == "dqn":
        model = DQN.load(config["export_path"] + model_name, device=DEVICE, normalize_images=False)
    elif model_name == "a2c":
        model = A2C.load(config["export_path"] + model_name, device=DEVICE, normalize_images=False)
    elif model_name == "ppo":
        model = PPO.load(config["export_path"] + model_name, device=DEVICE, normalize_images=False)
    else:
        print("Error, unknown model ({})".format(model_name), device=DEVICE, normalize_images=False)
