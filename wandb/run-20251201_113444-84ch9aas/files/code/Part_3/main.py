...
def train_model(env, model_name, thresh):
    ...
    if model_name == "dqn":
        ...
        model = PPO("CnnPolicy", env, verbose=1, tensorboard_log=f"runs/{run.id}", device=DEVICE, n_steps=4096, 
                    batch_size=2048,gamma=0.99, normalize_images = False)
    else:
...
env = make_env()
thresh = env.spec.reward_threshold if env.spec.reward_threshold is not None else -10000
# create environment
